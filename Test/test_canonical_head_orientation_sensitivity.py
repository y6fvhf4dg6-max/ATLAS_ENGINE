...
from CORE.atlas_canonical_head_orientation_sensitivity import (
    AtlasCanonicalHeadOrientationSensitivity,
)
...
def make_observation(**overrides):
    values = dict(
        ...
        orientation_id="face_up",
        reference_orientation_id="upright_reference",
        observation_state="OBSERVED",
        evidence_provenance="controlled print orientation comparison",
        affected_regions=(),
        sensitivity_state="NO_MATERIAL_CHANGE",
    )
    values.update(overrides)
    return AtlasCanonicalHeadOrientationSensitivity(**values)
...
def test_contract_is_frozen():
    ...
    with pytest.raises(dataclasses.FrozenInstanceError):
        observation.orientation_id = "face_down"
...
def test_orientation_ids_are_normalized():
    observation = make_observation(
        orientation_id=" Face Up ",
        reference_orientation_id=" Upright Reference ",
    )

    assert observation.orientation_id == "face_up"
    assert observation.reference_orientation_id == "upright_reference"
...
def test_contract_does_not_claim_support_requirement():
    observation = make_observation(
        affected_regions=(),
    )
