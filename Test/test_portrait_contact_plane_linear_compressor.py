...
def test_compressor_returns_float64_height_grid():
    result = AtlasPortraitContactPlaneLinearCompressor.compress(
        _projection_result(),
...
def test_compressor_scales_full_depth_to_target_height():
    result = AtlasPortraitContactPlaneLinearCompressor.compress(
        _projection_result(),
        target_maximum_height=0.2,
    )
...
def test_compressor_reports_effective_scale():
    result = AtlasPortraitContactPlaneLinearCompressor.compress(
        _projection_result(),
        target_maximum_height=0.2,
    )
    ...
    assert result["linear_scale"] == pytest.approx(
        0.25,
    )


def test_compressor_records_deterministic_metadata():
    result = AtlasPortraitContactPlaneLinearCompressor.compress(
        _projection_result(),
        target_maximum_height=0.2,
    )
...
def test_compressor_is_deterministic():
    ...
    first = AtlasPortraitContactPlaneLinearCompressor.compress(
        projection,
        target_maximum_height=0.2,
    )
    second = AtlasPortraitContactPlaneLinearCompressor.compress(
        projection,
        target_maximum_height=0.2,
    )
    ...
    assert first["linear_scale"] == pytest.approx(
        second["linear_scale"],
    )
