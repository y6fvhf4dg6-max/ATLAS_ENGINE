...
class AtlasCanonicalHeadJawChinQualityEvidence:
    criterion: str
    ...
    def __post_init__(self) -> None:
        ...
        if criterion not in self.CRITERIA:
            raise ValueError(
                "criterion must be one of the exact Item 9.3 criteria."
            )
        ...
        if evidence_status not in self.EVIDENCE_STATUSES:
            raise ValueError(
                "evidence_status must be one of the exact Item 9.3 evidence statuses."
            )
