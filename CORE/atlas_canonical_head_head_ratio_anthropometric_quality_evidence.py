...
class AtlasCanonicalHeadHeadRatioAnthropometricQualityEvidence:
    criterion: str
    ...
    def __post_init__(self) -> None:
        ...
        if criterion not in self.CRITERIA:
            raise ValueError(
                "criterion must be one of the exact Item 9.9 criteria."
            )
        ...
        if evidence_status not in self.EVIDENCE_STATUSES:
            raise ValueError(
                "evidence_status must be one of the exact Item 9.9 evidence statuses."
            )
        ...
        source_measurement = self._normalize_required_text(
            self.source_measurement,
            field_name="source_measurement",
        )
        semantic_scope = self._normalize_required_text(
            self.semantic_scope,
        ...
        object.__setattr__(self, "source_measurement", source_measurement)
        object.__setattr__(self, "semantic_scope", semantic_scope)
