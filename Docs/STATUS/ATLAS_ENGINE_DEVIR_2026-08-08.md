...
<!-- SPC_V1_DEVIR_AUTHORITY_2026_09_04 -->

# SPC V1 — Devir / Continuity Update
...
ATLAS has opened a new commercial validation path focused on:

> recognizable, stylized, aesthetically coherent and physically manufacturable personalized 3D collectibles generated from customer photographs.

This is a commercial pivot-validation program.

It does NOT erase the previous exact-identity research.

It does NOT retrospectively rewrite Phase 8 / Item 11 evidence.

It does NOT authorize Phase 9.

## Existing 15-item roadmap continuity

The existing roadmap remains preserved.
...
The exact-likeness work remains available as research evidence and may be reopened later by explicit decision.

Its current visual/technical failure state remains historically valid.
...
Their future disposition shall be updated progressively as SPC evidence clarifies whether each item should:

- remain unchanged;
- be resequenced;
- be adapted into SPC;
- remain research-only;
- be superseded by a new commercial dependency.

No such decision is made automatically.
...
SPC V1 does not authorize or start Phase 9.

## SPC V1 roadmap authority set

The new roadmap is distributed across exactly three primary files:
...
These three files together form the SPC V1 roadmap authority.
...
Only `CHIBI` is authorized for the first end-to-end feasibility path.

Stylized Bust remains downstream.

Bobblehead remains downstream.

Initial Bobblehead, when reached, shall first validate a fixed-head product before any spring/bobble mechanism.
...
A single-front-image-to-3D path may be benchmarked as a comparison route.

It is not currently the preferred production architecture.
...
Human support is allowed during validation.

Human work is not hidden.

Every meaningful intervention should eventually be:

- categorized;
- timed;
- evaluated as a candidate for future ATLAS automation.

## Gate chain

Current major gate order:

1. `SPC-C0` — Commercial / Legal Eligibility
2. `SPC-C1` — Chibi Product Definition
3. `SPC-G1` — 2D Stylized Identity
4. `SPC-G2` — Multi-View Identity Consistency
5. `SPC-G3` — 3D Identity Transfer
6. `SPC-G4` — ATLAS Normalization Feasibility
7. `SPC-G5` — Physical Product
8. `SPC-G6` — Economic Feasibility
9. `SPC-G7` — Pivot Lock

No downstream stage should bypass the required upstream gate.
...
with sufficient technical AND commercial evidence.

## Immediate next work

The next execution sequence starts at the front of the dependency chain:

1. Commercial/legal AI eligibility audit.
2. Chibi Style Bible V0.
3. Identity Anchor Contract.
4. Initial physical Chibi contract.
5. Customer Capture Contract.
6. Bounded 2D stylized identity benchmark.
7. Human 2D identity decision.
8. Identity-consistent multi-view.
9. Multi-view gate.
10. Only then 3D benchmark.

## Important continuity rule

Do not resume the previous exact-likeness solver path merely because it remains in the repository.

Do not delete the exact-likeness evidence.

Do not rewrite it as a success.

Do not start Phase 9.

Do not implement Bust or Bobblehead before Chibi produces sufficient validated evidence.

Do not skip the legal/commercial eligibility gate before using external AI services with real customer photographs.

Do not treat beautiful AI output as production-ready geometry.

## Devir instruction for a new engine/session

When continuing ATLAS SPC work, first read:

1. `Docs/START_HERE.md`
2. `Docs/STATUS/CURRENT_STATUS.md`
3. `Docs/STATUS/ATLAS_ENGINE_DEVIR_2026-08-08.md`
4. `Docs/Roadmap/SPC_V1_PART_1_STRATEGY_AND_IDENTITY.md`
5. `Docs/Roadmap/SPC_V1_PART_2_3D_ATLAS_AND_PRODUCT.md`
6. `Docs/Roadmap/SPC_V1_PART_3_BUSINESS_PILOT_AND_GOVERNANCE.md`
...
- first product = `CHIBI`
- product order = `1 -> 3 -> 2`
- exact identity = `RESEARCH_PARKED / NON_MVP_BLOCKING`
- Items 12–15 = `HOLD_PENDING_SPC_VALIDATION`
...
[PF1_FEMALE_FINAL_NORMAL_PROPORTION_CLOSURE_2026_09_10:BEGIN]
CURRENT_WORK_PROGRAM = P-3
CURRENT_GATE = GATE-P3
PF1_FEMALE_FULL_BODY_ASSEMBLY = COMPLETE / PASS
PF1_FEMALE_ITEM16_ANATOMICAL_NECK_SHOULDER_CONTINUITY = PASS / LOCKED
PF1_FEMALE_ITEM17_TOPOLOGY = PASS / LOCKED
PF1_FEMALE_ITEM18_HUMAN_VISUAL_GATE = PASS / HUMAN ACCEPTED
PF1_FEMALE_ITEM19_FINAL_CANDIDATE = PASS / SELECTED
PF1_FEMALE_ITEM20_PERSISTENCE_REPRODUCIBILITY = PASS / CLOSED
PF1_FEMALE_FINAL_CANDIDATE = atlas_pf1_female_final_normal_proportion_v6.glb
PF1_FEMALE_FINAL_CANDIDATE_SHA256 = 05d13f37955c22c808adc152bddaa32ed9240e1da093f39cb7157d948b4f7fa2
PF1_FEMALE_FINAL_EVIDENCE_DIR = EVIDENCE/PERSONALIZED_FIGURINE/P3/female_final_normal_proportion_candidate_2026_09_10
PF1_FEMALE_FINAL_GEOMETRY_OPTIMIZATION = STOPPED / LOCKED
PF1_FEMALE_RODIN_V2_5_HEAD = KEEP
PF1_FEMALE_MAKEHUMAN_BODY = KEEP
PF1_FEMALE_BODY_HEAD_RATIO = 8.25 / KEEP
PF1_FEMALE_RIGID_PLACEMENT = KEEP
PF1_FEMALE_EYE_FACE_GEOMETRY_MODIFIED_BY_V6 = NO
PF1_FEMALE_PRIOR_3Q_EYE_ANOMALY = RENDER_DEPTH_ORDER_ARTIFACT / NO_GEOMETRY_DEFECT_PROVEN
